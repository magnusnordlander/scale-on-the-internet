...
void ScaleWrapper::refresh() {
    ...
    this->latestValueShortAverage = this->averagePointSince(SHORT_AVG_MS*1000);
    this->latestValueLongAverage = this->averagePointSince(LONG_AVG_MS*1000);
    ...
    Serial.print(" Stable lo: ");
    Serial.print(this->isValueStableLowAccuracy() ? "Y" : "N");
    Serial.print(" Stable hi: ");
    Serial.println(this->isValueStableHighAccuracy() ? "Y" : "N");
*/

    if (this->tareValue == 0 && this->isValueStableHighAccuracy()) {
        this->tare(TARE_TIME_MS*1000);
    }
}
...
float ScaleWrapper::_getRateOfChange() {
    ...
    MeasuringPoint first = this->averagePointSince(ROC_MS*1000, ROC_NUM);
    MeasuringPoint last = this->latestAverage(ROC_NUM);
...
bool ScaleWrapper::isValueStableHighAccuracy() {
    return this->isValueStable(STABILITY_HIGH_RES_MS*1000, STABILITY_HIGH_RES_MIN_NUM, STABILITY_HIGH_RES_SIGMA);
}

bool ScaleWrapper::isValueStableLowAccuracy() {
    return this->isValueStable(STABILITY_LOW_RES_MS*1000, STABILITY_LOW_RES_MIN_NUM, STABILITY_LOW_RES_SIGMA);
}
